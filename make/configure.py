...
class Project( Action ):
    ...
    def _action( self ):
        ...
        if repo.type == 'release':
            ...
            if repo.branch != '':
                self.version = '%d.%d.%d-%d-%s-%s (x264-r2555-0c21480)' % (self.vmajor, self.vminor,
                    self.vpoint, repo.rev, repo.shorthash, repo.branch)
            else:
                self.version = '%d.%d.%d-%d-%s (x264-r2555-0c21480)' % (self.vmajor, self.vminor,
                    self.vpoint, repo.rev, repo.shorthash)
            url_ctype = '_unstable'
            url_ntype = 'unstable'
            self.build = time.strftime('%Y%m%d') + '01'
            self.title = '%s %s (%s) (x264-r2555-0c21480)' % (self.name,self.version,self.build)
